...
app.whenReady().then(async () => {
    ...
    ipcMain.handle('scan-fonts', async () => {
        try {
                    ...
                    for (const item of items) {
                        ...
                        if (item.isDirectory()) {
                            ...
                            try {
                                ...
                                try { importedMetadata = JSON.parse(fs.readFileSync(path.join(fullPath, 'font-metadata.json'), 'utf8')); } catch (_) {}

                                for (const fontFile of files) {
                                    const ext = path.extname(fontFile).toLowerCase();
                                    if (!fontExts.has(ext)) continue;
                                    const meta = importedMetadata[fontFile] || inferWeightAndStyle(fontFile);
                                    results.push({
                                        family: meta.family || familyName,
            ...
            // Personal fonts are local development assets, separate from distributable resources.
            if (!app.isPackaged) fontList.push(...scanDir(path.join(__dirname, '..', 'fonts-local'), false));
